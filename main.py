...
if fl is not None:
    ...
    # Specify the full path for reading the file
    ...
    linechart = pd.DataFrame(
        filtered_df.groupby(filtered_df["month_year"].dt.strftime("%Y : %b"))[
            "Sales"
        ].sum()
    ).reset_index()
    ...
    st.warning("Please make sure the uploaded file contains the 'Order Date' column.")
